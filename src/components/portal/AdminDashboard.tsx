...
import {
  ...
  UserCheck, ShoppingBag, MessageSquare,
} from 'lucide-react';
import {
...
export default function AdminDashboard() {
  ...
  const fetchOverview = useCallback(async (showLoader = false) => {
    ...
    try {
      ...
      setRecentContactMessages(data.recentContactMessages || []);
      setError('');
    } catch {
  ...
  useEffect(() => {
    fetchOverview(true);
    const id = window.setInterval(() => fetchOverview(false), 30000);
    const onFocus = () => fetchOverview(false);
  ...
  const fmtRupee = (n: number) => `Rs. ${fmt(n)}`;
  ...
  const stats = [
    ...
    { icon: MessageSquare, label: 'Contact Messages',        value: fmt(overview?.totalContactMessages),color: 'bg-cyan-500',    delay: 0.25, href: '/admin/contact-messages', trend: `${fmt(overview?.newContactMessages)} new` },
    { icon: GraduationCap, label: 'Online Enrollments',      value: fmt(overview?.totalEnrolled),       color: 'bg-[#ffa800]',   delay: 0.3,  href: '/admin/online-enrollments' },
    { icon: ShoppingBag,   label: 'Purchased Courses',       value: fmt(overview?.purchasedCourses),    color: 'bg-teal-500',    delay: 0.35, href: '/admin/purchased-courses' },
    { icon: Award,         label: 'Certificates Issued',     value: fmt(overview?.totalCertificates),   color: 'bg-indigo-500',  delay: 0.4,  href: '/admin/certificates' },
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
          ...
          {revenueChart.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={revenueChart}>
                ...
                <Tooltip
                  formatter={(v: any) => [`Rs. ${v.toLocaleString('en-IN')}`, 'Revenue']}
      ...
      {recentContactMessages.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-semibold text-gray-900">Recent Contact Messages</h3>
              <p className="text-xs text-gray-400 mt-0.5">Latest submissions from the contact page</p>
            </div>
            <Link href="/admin/contact-messages" className="text-xs text-[#ffa800] hover:underline font-medium">
              View all -&gt;
            </Link>
          </div>
          <div className="space-y-3">
            {recentContactMessages.slice(0, 6).map((item) => (
              <Link key={item.id} href="/admin/contact-messages" className="flex items-center gap-3 rounded-xl hover:bg-gray-50 transition-colors">
                <div
                  className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
      ...
      >
        {[
          ...
          { label: 'Contact Messages',    href: '/admin/contact-messages',    icon: MessageSquare,  color: 'bg-cyan-50 text-cyan-600 border-cyan-100' },
