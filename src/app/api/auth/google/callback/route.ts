...
import { authCookieOptions, getClientIp } from '@/lib/security';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    await attachLoginSession(response, request, user, 7 * 24 * 60 * 60);
    response.cookies.set(STATE_COOKIE, '', { ...authCookieOptions(0, 'lax'), maxAge: 0 });
    return response;
  } catch (error) {
