...
function LoginContent() {
  ...
  const handleGoogleSignIn = () => {
    ...
    if (searchParams.get('from') === 'buy') {
      const redirect = new URLSearchParams();
      const prog = searchParams.get('program');
      const amt = searchParams.get('amount');
      if (prog) redirect.set('program', prog);
      if (amt) redirect.set('amount', amt);
      p.set('redirect', `/dashboard?${redirect.toString()}`);
    } else {
      p.set('redirect', '/');
    }
          ...
          <div className="px-8 py-8">
            ...
            <div className="text-center mb-8">
              <Link href="/" className="inline-flex items-center justify-center gap-2 mb-5 group">
                <Image src="/newadylogo.png" alt="Adyapan" width={40} height={40} className="h-10 w-auto group-hover:scale-105 transition-transform" />
