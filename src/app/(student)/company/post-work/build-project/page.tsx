  ...
  return (
    <div className="space-y-5">
      <div className="bg-white border-2 border-gray-100 rounded-2xl p-5">
        ...
        <Row icon={DollarSign} label="Budget" value={`Rs. ${Number(form.budget).toLocaleString('en-IN')}`} />
        {form.techPreference && (
      ...
      <div className="bg-orange-50 border-2 border-orange-200 rounded-2xl p-5">
        <div className="flex items-center justify-between">
          ...
          <p className="text-3xl font-extrabold text-orange-500">
            Rs. {Number(form.budget).toLocaleString('en-IN')}
          </p>
        </div>
      </div>
    </div>
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const options = {
        ...
        /* â"€â"€ 4. On payment success -> verify server-side â"€â"€ */
...
export default function BuildProjectPage() {
  ...
  const validateStep1 = useCallback((): boolean => {
    ...
    if (!form.budget) {
      ...
      e.budget = 'Minimum project submission amount is Rs. 3000';
    }
