...
async function sendViaResend(to: string, subject: string, html: string, text: string): Promise<boolean> {
  ...
  if (!config) {
    console.warn('[PasswordResetEmail] RESEND_API_KEY not configured.');
    return false;
  }
  ...
  try {
    ...
    if (result.error) {
      console.error('[PasswordResetEmail] Resend error:', result.error);
      return false;
    }
    return true;
  } catch (err: any) {
    console.error('[PasswordResetEmail] Resend failed:', err?.message);
    return false;
  }
}
...
export async function sendPasswordResetEmail({
  ...
  const subject = 'Reset your Adyapan password';
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:24px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.10);">
  <tr>
    <td style="background:linear-gradient(135deg,#ffa800 0%,#ff6b00 100%);padding:36px 40px 30px;text-align:center;">
      <h1 style="margin:0 0 8px;color:#ffffff;font-size:28px;font-weight:800;">Reset Your Password</h1>
      <p style="margin:0;color:rgba(255,255,255,0.9);font-size:15px;">This secure link is valid for 15 minutes.</p>
    </td>
  </tr>
  <tr><td style="padding:34px 40px 0;">
    ...
    <table cellpadding="0" cellspacing="0" style="margin:0 auto 28px;">
      <tr>
        <td align="center" style="border-radius:14px;background:linear-gradient(135deg,#ffa800,#ff6b00);box-shadow:0 4px 20px rgba(255,107,0,0.35);">
          <a href="${safeResetUrl}" style="display:inline-block;padding:15px 36px;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;border-radius:14px;">Reset Password</a>
        </td>
      </tr>
    </table>
    ...
    <p style="margin:0 0 28px;word-break:break-all;font-size:12px;color:#ea580c;line-height:1.6;">${safeResetUrl}</p>
    <p style="margin:0 0 34px;font-size:13px;color:#9ca3af;line-height:1.7;">If you did not request this, you can safely ignore this email.</p>
  </td></tr>
  <tr>
...
We received a request to reset your Adyapan account password.
...
export function getPasswordResetUrl(token: string, requestOrigin?: string): string {
